...
const NavbarSimple: FC = () => {
  ...
  const navList = (
    <ul className="mb-4 mt-2 flex flex-col gap-2 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
      <Typography
        ...
        variant="small"
        color="blue-gray"
        className="p-1 font-medium hover:text-red-600 transition-colors"
      >
        ...
        >
          <Link to="/cart" className="flex items-center gap-1 group">
            <div className="relative">
              ...
              {cartCount > 0 && (
                <span className="absolute -top-5 right-3 bg-red-600 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] flex items-center justify-center border border-white">
                  {cartCount}
                </span>
              )}
            </div>
      ...
      {!token ? (
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 border-t lg:border-t-0 pt-2 lg:pt-0">
          <div className="flex items-center gap-2 px-3 py-1 bg-gray-100 rounded-full">
            <UserCircleIcon size={20} className="text-red-600" />
            <Typography
              variant="small"
              className="font-bold text-blue-gray-900 uppercase"
            >
              {user?.username || "User"}
            </Typography>
          </div>
          ...
          <Typography
            ...
            variant="small"
            color="red"
            className="p-1 font-medium cursor-pointer flex items-center gap-1 hover:opacity-80 transition-opacity"
  ...
  // ---------------------------------------------------
  return (
    <Navbar className="sticky top-0 z-50 h-max max-w-full rounded-none py-2 px-4 lg:px-8 lg:py-4 shadow-sm">
      <div className="container mx-auto flex items-center justify-between text-blue-gray-900">
        <Link to="/" className="mr-4 cursor-pointer py-1.5 flex items-center">
          <svg
            width="111"
            height="34"
            viewBox="0 0 111 34"
